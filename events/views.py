...
from .serializers import EventSerializer, CategorySerializer, BookingSerializer, TicketSerializer, TicketTierSerializer
from rest_framework.decorators import api_view, permission_classes
from rest_framework.permissions import IsAuthenticated
from .mpesa_utils import MpesaHandler 
from django.shortcuts import get_object_or_404
from .tasks import generate_event_tickets
from drf_spectacular.utils import extend_schema, extend_schema_view
from rest_framework.parsers import MultiPartParser, FormParser

# Create your views here.
@extend_schema_view(
    create=extend_schema(request={'multipart/form-data': EventSerializer}),
    update=extend_schema(request={'multipart/form-data': EventSerializer}),
    partial_update=extend_schema(request={'multipart/form-data': EventSerializer}),
)
...
@extend_schema(responses={200: dict}, methods=['POST']) # Tells Swagger what to expect
@api_view(['POST'])
...
def mpesa_callback(request):
    ...
    if result_code == 0:
        ...
        booking.save()
        # Queue the ticket generation
        transaction.on_commit(lambda: generate_event_tickets.delay(booking.id))
        
    return Response({"ResultCode": 0, "ResultDesc": "Accepted"})

class TicketViewSet(viewsets.ReadOnlyModelViewSet):
    '''
    This allows users to see their tickets.
    We use readonly because tickets are generated by Celery, not users
    '''
    queryset = Ticket.objects.all()
    serializer_class = TicketSerializer
    
    def get_queryset(self):
        # High-performance filter: only show tickets belonging to the logged-in user
